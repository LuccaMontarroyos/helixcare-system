angular.module("helixcare.appointments").controller("AppointmentsController", [
  ...
  "AuthService",
  "ToastService",
  function ($scope, $filter, AppointmentsService, PatientsService, AuthService, ToastService) {

    $scope.currentUser  = AuthService.getCurrentUser();
    $scope.appointments = [];
    $scope.isLoading    = true;
    $scope.isDoctorUser = $scope.currentUser && $scope.currentUser.role === "DOCTOR";
    ...
    var MONTHS_LONG = ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];
    ...
    function _dayUTCBounds(localDate) {
      var start = new Date(localDate.getFullYear(), localDate.getMonth(), localDate.getDate(), 0, 0, 0, 0);
      var end   = new Date(localDate.getFullYear(), localDate.getMonth(), localDate.getDate(), 23, 59, 59, 999);
    ...
    function _weekUTCBounds(anchorDate) {
      ...
      var start = new Date(anchorDate.getFullYear(), anchorDate.getMonth(), anchorDate.getDate(), 0, 0, 0, 0);
      var end   = new Date(weekEnd.getFullYear(), weekEnd.getMonth(), weekEnd.getDate(), 23, 59, 59, 999);
      return { start: start.toISOString(), end: end.toISOString() };
    }
    ...
    function _buildWeekDays(anchorDate) {
      ...
      var days     = [];
      for (var i = 0; i < 7; i++) {
        var d      = _localDate(anchorDate.getFullYear(), anchorDate.getMonth(), anchorDate.getDate() + i);
      ...
      var first = days[0].date, last = days[6].date;
      var sameMonth = first.getMonth() === last.getMonth();
      $scope.week.label = sameMonth
        ? first.getDate() + " – " + last.getDate() + " " + MONTHS_LONG[last.getMonth()] + " " + last.getFullYear()
        : first.getDate() + " " + MONTHS_LONG[first.getMonth()] + " – " + last.getDate() + " " + MONTHS_LONG[last.getMonth()] + " " + last.getFullYear();
      return days;
    }

    function _applyCountsToWeek() {
      angular.forEach($scope.week.days, function (day) { day.count = $scope.week.counts[day.iso] || 0; });
    }
    ...
    function _loadWeekCounts() {
      ...
      if ($scope.filters.patient_id) extra.patient_id = $scope.filters.patient_id;
      if ($scope.filters.status)     extra.status      = $scope.filters.status;
      AppointmentsService.getWeekCounts(bounds.start, bounds.end, extra)
        .then(function (counts) { $scope.week.counts = counts; _applyCountsToWeek(); })
        .catch(angular.noop)
        .finally(function () { $scope.week.isLoadingCounts = false; });
    }
    ...
    $scope.prevWeek = function () {
      $scope.week.anchorDate  = _localDate($scope.week.anchorDate.getFullYear(), $scope.week.anchorDate.getMonth(), $scope.week.anchorDate.getDate() - 7);
      $scope.week.selectedISO = null;
      _applyWeekFilter(); _rebuildWeek(); $scope.loadAppointments();
    };
    ...
    $scope.jumpToDate = function () {
      ...
      if (!raw) return;
      var target = (raw instanceof Date)
        ? _localDate(raw.getFullYear(), raw.getMonth(), raw.getDate())
        : (function() { var p = String(raw).split("-"); return _localDate(parseInt(p[0],10), parseInt(p[1],10)-1, parseInt(p[2],10)); }());
    ...
    $scope.applyFilters = function () {
      $scope.filters.page = 1; $scope.loadAppointments();
    };

    $scope.clearFilters = function () {
      $scope.filters.page = 1; $scope.filters.status = "";
      $scope.filters.patient_id = ""; $scope.filters.doctor_id = "";
      $scope.typeahead.patient.term = ""; $scope.typeahead.doctor.term = "";
      $scope.week.jumpDate = null; $scope.week.selectedISO = null;
      $scope.week.anchorDate = _getWeekStart(new Date());
      _applyWeekFilter(); _rebuildWeek(); $scope.loadAppointments();
    };
    ...
    $scope.searchPatients = function () {
      if (!$scope.typeahead.patient.term) {
        $scope.typeahead.patient.isOpen = false; $scope.filters.patient_id = ""; $scope.applyFilters(); return;
      }
      $scope.typeahead.patient.loading = true;
      PatientsService.getPatients({ search: $scope.typeahead.patient.term })
        .then(function (res) { $scope.typeahead.patient.results = res.items || res.data || res; $scope.typeahead.patient.isOpen = true; })
        .finally(function () { $scope.typeahead.patient.loading = false; });
    };

    $scope.selectPatient = function (patient) {
      $scope.typeahead.patient.term = patient.name; $scope.filters.patient_id = patient.id;
      $scope.typeahead.patient.isOpen = false; _loadWeekCounts(); $scope.applyFilters();
    };

    $scope.searchDoctors = function () {
      if (!$scope.typeahead.doctor.term) {
        $scope.typeahead.doctor.isOpen = false; $scope.filters.doctor_id = ""; $scope.applyFilters(); return;
      }
      $scope.typeahead.doctor.loading = true;
      AppointmentsService.searchUsers($scope.typeahead.doctor.term)
        .then(function (res) { $scope.typeahead.doctor.results = res.items || res.data || res; $scope.typeahead.doctor.isOpen = true; })
        .finally(function () { $scope.typeahead.doctor.loading = false; });
    };

    $scope.selectDoctor = function (doctor) {
      $scope.typeahead.doctor.term = doctor.name; $scope.filters.doctor_id = doctor.id;
      $scope.typeahead.doctor.isOpen = false; _loadWeekCounts(); $scope.applyFilters();
    };
    ...
    function generatePageNumbers() {
      ...
      var endPage   = Math.min($scope.pagination.totalPages, startPage + maxPages - 1);
    ...
    $scope.loadAppointments = function () {
      ...
      AppointmentsService.getAppointments(queryParams)
        ...
        .catch(function () { ToastService.error("Falha ao carregar a agenda."); })
        .finally(function () { $scope.isLoading = false; });
    };

    $scope.goToPage = function (pageNumber) {
      if (pageNumber < 1 || pageNumber > $scope.pagination.totalPages || pageNumber === $scope.filters.page) return;
      $scope.filters.page = pageNumber; $scope.loadAppointments();
    };

    // =========================================================================
    // Ações de status — cada transição explícita e nomeada
    // =========================================================================

    // Utilitário interno para qualquer mudança de status
    function _changeStatus(appointment, newStatus, successMsg) {
      ...
      var payload = {
        patient_id:       appointment.patient ? appointment.patient.id : appointment.patient_id,
        doctor_id:        appointment.doctor  ? appointment.doctor.id  : appointment.doctor_id,
    ...
    // CONFIRMED → WAITING (paciente chegou à recepção)
    $scope.markAsWaiting = function (appt) {
      _changeStatus(appt, "WAITING", "Paciente registrado na sala de espera.");
    };

    // CONFIRMED → NO_SHOW (paciente faltou — ação manual)
    $scope.markAsNoShow = function (appt) {
      if (!confirm("Confirmar falta do paciente " + appt.patient.name + "?")) return;
      _changeStatus(appt, "NO_SHOW", "Falta registrada para " + appt.patient.name + ".");
    };
    ...
    $scope.saveReschedule = function () {
      if (!$scope.rescheduleForm.date || !$scope.rescheduleForm.time) {
        ToastService.warning("Informe a nova data e hora."); return;
      }
      $scope.isRescheduling = true;

      var raw = $scope.rescheduleForm.date;
      var baseDate = (raw instanceof Date)
        ? _localDate(raw.getFullYear(), raw.getMonth(), raw.getDate())
        : (function() { var p = String(raw).split("-"); return _localDate(parseInt(p[0],10),parseInt(p[1],10)-1,parseInt(p[2],10)); }());
      ...
      if (angular.isString(timeVal)) {
        ...
        baseDate.setHours(parseInt(parts[0],10)||0, parseInt(parts[1],10)||0, 0, 0);
      } else {
        var t = new Date(timeVal); baseDate.setHours(t.getHours(), t.getMinutes(), 0, 0);
      }
      ...
      AppointmentsService.rescheduleAppointment(
        ...
        .finally(function () { $scope.isRescheduling = false; });
    };
    ...
    $scope.openModal = function (appt) {
      $scope.isSaving = false;
      $scope.modalTypeahead.patient = { term: "", results: [], isOpen: false, loading: false };
      $scope.modalTypeahead.doctor  = { term: "", results: [], isOpen: false, loading: false };

      if (appt) {
        $scope.isEditMode = true;
        var d = new Date(appt.appointment_date);
        $scope.appointmentForm = {
          id: appt.id, patient_id: appt.patient.id,
    ...
    $scope.searchModalPatients = function () {
      ...
      PatientsService.getPatients({ search: $scope.modalTypeahead.patient.term })
        .then(function (res) { $scope.modalTypeahead.patient.results = res.items || res.data || res; $scope.modalTypeahead.patient.isOpen = true; })
    ...
    $scope.searchModalDoctors = function () {
      ...
      AppointmentsService.searchUsers($scope.modalTypeahead.doctor.term)
        .then(function (res) { $scope.modalTypeahead.doctor.results = res.items || res.data || res; $scope.modalTypeahead.doctor.isOpen = true; })
    ...
    $scope.saveAppointment = function () {
      ...
      if (angular.isString(timeVal)) {
        var parts = timeVal.split(":"); baseDate.setHours(parseInt(parts[0],10)||0, parseInt(parts[1],10)||0, 0, 0);
      } else { var t = new Date(timeVal); baseDate.setHours(t.getHours(), t.getMinutes(), 0, 0); }

      var payload = {
        patient_id: $scope.appointmentForm.patient_id, doctor_id: $scope.appointmentForm.doctor_id,
        appointment_date: baseDate.toISOString(), notes: $scope.appointmentForm.notes || "",
      };
      if ($scope.isEditMode) payload.status = $scope.appointmentForm.status;

      var request = $scope.isEditMode
        ? AppointmentsService.updateAppointment($scope.appointmentForm.id, payload)
      ...
      request
        .then(function () {
          ...
          $scope.closeModal(); _loadWeekCounts(); $scope.loadAppointments();
        })
        .catch(function (err) {
    ...
    // =========================================================================
    // Helpers de status — configuração centralizada para todos os 8 status
    // =========================================================================
    $scope.getStatusConfig = function (status) {
      var config = {
        SCHEDULED:   { class: "bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400",                 icon: "schedule",     text: "Agendado"       },
        CONFIRMED:   { class: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400",              icon: "check_circle", text: "Confirmado"     },
        WAITING:     { class: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400",          icon: "chair",        text: "Aguardando"     },
        IN_PROGRESS: { class: "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400",      icon: "stethoscope",  text: "Em Atendimento" },
        COMPLETED:   { class: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400",  icon: "task_alt",     text: "Concluído"      },
        NO_SHOW:     { class: "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400",              icon: "person_off",   text: "Faltou"         },
        RESCHEDULED: { class: "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400",             icon: "update",       text: "Remarcado"      },
        CANCELED:    { class: "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400",                  icon: "cancel",       text: "Cancelado"      },
      };
      return config[status] || { class: "bg-slate-100 text-slate-500", icon: "help", text: status };
    };

    // Status terminais — sem ações e visual reduzido
    $scope.isTerminalStatus = function (status) {
      return ["COMPLETED", "NO_SHOW", "RESCHEDULED", "CANCELED"].indexOf(status) !== -1;
    };
